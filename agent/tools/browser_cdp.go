...
// NewBrowserCDPToolWithRelay Create enhanced CDP browser tool with Relay support
func NewBrowserCDPToolWithRelay(headless bool, timeout int, relayURL, relayMode string) *BrowserCDPTool {
	return &BrowserCDPTool{
		BrowserTool: NewBrowserToolWithRelay(headless, timeout, relayURL, relayMode),
	}
}
...
func (b *BrowserCDPTool) BrowserPrintToPDF(ctx context.Context, params map[string]interface{}) (string, error) {
	...
	if p, ok := params["print_background"].(bool); ok {
		...
		printBackground = true // 默认打印背景
	}

	slog.Debug("Browser generating PDF",
		"url", urlStr,
		"landscape", landscape,
...
func (b *BrowserCDPTool) BrowserExtractStructuredData(ctx context.Context, params map[string]interface{}) (string, error) {
	...
	slog.Debug("Browser extracting structured data",
		"url", urlStr,
		"type", extractType)
...
func (b *BrowserCDPTool) buildExtractionScript(extractType string) string {
	...
	default:
		return `(function() { return JSON.stringify({}, null, 2); })();`
	}
}
...
func (b *BrowserCDPTool) BrowserEmulateDevice(ctx context.Context, params map[string]interface{}) (string, error) {
	...
	slog.Debug("Browser emulating device", "device", device)
...
func (b *BrowserCDPTool) BrowserSetViewport(ctx context.Context, params map[string]interface{}) (string, error) {
	...
	slog.Debug("Browser setting viewport",
		"width", width,
		"height", height)
...
// BrowserGetCookies Get all cookies for the current page
func (b *BrowserCDPTool) BrowserGetCookies(ctx context.Context, params map[string]interface{}) (string, error) {
	sessionMgr := GetBrowserSession()
	if !sessionMgr.IsReady() {
		return "", fmt.Errorf("browser session not ready")
	}
