...
const logLevelConfig = {
  debug: { color: 'text-ocean-depth/50', bg: 'bg-ocean-depth/10', label: 'DEBUG' },
  info: { color: 'text-cyan-electric', bg: 'bg-cyan-electric/15', label: 'INFO' },
  warn: { color: 'text-amber-500', bg: 'bg-amber-500/15', label: 'WARN' },
  error: { color: 'text-red-500', bg: 'bg-red-500/15', label: 'ERROR' },
}
    ...
    <div className="glass-card-solid h-full flex flex-col overflow-hidden">
      ...
      <div className="flex items-center justify-between px-4 py-3 border-b border-cyan-electric/15">
        <div className="flex items-center gap-3">
          ...
          <h2 className="font-display font-semibold text-ocean-deep">System Logs</h2>
          <span className="text-xs font-mono text-ocean-depth/50 px-2 py-0.5 rounded bg-ocean-depth/10">
      ...
      {/* Filters */}
      <div className="px-4 py-2 border-b border-cyan-electric/15 flex items-center gap-4 flex-wrap">
        {/* Level Filter */}
        <div className="flex items-center gap-2">
          <Filter size={14} className="text-ocean-depth/40" />
              ...
              <motion.button
                ...
                className={`px-2 py-1 rounded text-xs font-mono transition-colors ${
                  levelFilter === level
                    ...
                    : 'text-ocean-depth/50 hover:text-ocean-depth/70 hover:bg-ocean-depth/10'
                }`}
                whileTap={{ scale: 0.95 }}
              >
                {level.toUpperCase()}
      ...
      {/* Log Terminal */}
      <div className="flex-1 overflow-y-auto font-mono text-sm bg-ocean-depth/5">
        <AnimatePresence initial={false}>
          {displayLogs.map((log) => (
            ...
            >
              ...
              {/* Source */}
              <span className="text-cyan-electric/70 min-w-[80px]">
                [{log.source}]
              </span>
